...
public class SelectBrdService {
	 ...
	 * @throws Exception
	 */
	public BrdPagination getBrdPagination(int cp, String brdType) throws Exception {
		
		Connection conn = getConnection();
		
		// DB에서 전체 게시글 수 + 카테고리 이름을 얻어옴
		Map<String, Object> map = dao.getBrdListCount(conn, brdType);
		
		close(conn);
		
		int brdListCount = (int)map.get("brdListCount");
		String brdTypeNm = (String)map.get("brdTypeNm");
	
		return new BrdPagination(cp, brdListCount, brdType, brdTypeNm);
	}
